...
//       if (row && row[0] !== undefined && String(row[0]).trim().toLowerCase() === code.trim().toLowerCase()) {
//         return true;
//       }
//     }
//     return false;
...
//         `${GOOGLE_SCRIPT_WEB_APP_URL}?action=readData&sheetName=${encodeURIComponent(sheetName.trim())}`
//       );
...
export default function App() {
  ...
  const [lastSync, setLastSync] = React.useState<Date | null>(null);
  const [autoRefresh, setAutoRefresh] = React.useState(false);
  const [clearBeforeFetch, setClearBeforeFetch] = React.useState(false);
  ...
  const validateInput = (): boolean => {
    ...
    if (!sheetName.trim()) {
      showToast("error", "Please select a sheet");
      return false;
    }
  ...
  const loadSheetNames = async () => {
    try {
      ...
      showToast("error", error.message || "Failed to load sheets");
    }
  };
  ...
  const getUsedDataRange = async (sheet: any, context: any) => {
    const usedRange = sheet.getUsedRangeOrNullObject(true);
    usedRange.load("isNullObject,rowIndex,columnIndex,rowCount,columnCount,values");
    await context.sync();
    return usedRange;
  };

  const findItemRow = async (context: any, code: string): Promise<number> => {
    const sheet = await getTargetWorksheet(context);
    const usedRange = await getUsedDataRange(sheet, context);

    if (usedRange.isNullObject) return -1;

    const values = usedRange.values || [];
    for (let i = 0; i < values.length; i++) {
      const row = values[i];
      if (row && row[0] !== undefined && String(row[0]).trim().toLowerCase() === code.trim().toLowerCase()) {
        return usedRange.rowIndex + i;
      }
    }
    return -1;
  };

  const getNextEmptyRow = async (context: any): Promise<number> => {
    const sheet = await getTargetWorksheet(context);
    const usedRange = sheet.getUsedRangeOrNullObject(true);
    usedRange.load("isNullObject,rowIndex,rowCount");
    await context.sync();

    if (usedRange.isNullObject) return 0;
  ...
  const pushToSheets = async () => {
    ...
    try {
      ...
      const rowIndex = await Excel.run(async (context: any) => {
        return await findItemRow(context, itemCode);
      });
      ...
      if (result.status === "success") {
        showToast(rowIndex >= 0 ? "success" : "success", rowIndex >= 0 ? "Row updated!" : "Data sent!");
        setItemCode("");
        setQuantity("");
        setLastSync(new Date());
      } else {
      ...
      if (result.status === "success" && result.data && result.data.length > 0) {
        await Excel.run(async (context: any) => {
          ...
          if (clearBeforeFetch) {
            ...
            usedRange.load("isNullObject");
            await context.sync();
            if (!usedRange.isNullObject) usedRange.clear(Excel.ClearApplyTo.contents);
          }
  ...
  return (
    ...
    >
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "90%", mt: 1 }}>
        <Typography sx={{ fontSize: "11px", fontWeight: 600, color: "#475569" }}>Auto-refresh (30s)</Typography>
        <Button
          size="small"
          variant={autoRefresh ? "contained" : "outlined"}
          onClick={() => setAutoRefresh(!autoRefresh)}
          disabled={loading}
          sx={{ fontSize: "10px", py: 0.4, px: 1.5, minWidth: "auto", textTransform: "none", height: 24 }}
        >
          {autoRefresh ? "ON" : "OFF"}
        </Button>
      </Box>
      ...
      <Box sx={{ display: "flex", justifyContent: "space-between", alignItems: "center", width: "90%", mt: 0 }}>
        <Typography sx={{ fontSize: "11px", fontWeight: 600, color: "#475569" }}>Clear before fetch</Typography>
        <Button
          size="small"
          variant={clearBeforeFetch ? "contained" : "outlined"}
          onClick={() => setClearBeforeFetch(!clearBeforeFetch)}
